...
int main(int argc, char *argv[]) 
{
  ...
  double **sum_fraction_gt_cutoff = (double**)alloc_chk(MAX_SPECIES*sizeof(double*)); for(i=0;i<MAX_SPECIES;i++) sum_fraction_gt_cutoff[i]=(double*)alloc_chk(MAX_SPECIES*sizeof(double));;
  long **count_fraction_gt_cutoff = (long**)alloc_chk(MAX_SPECIES*sizeof(long*)); for(i=0;i<MAX_SPECIES;i++) count_fraction_gt_cutoff[i]=(long*)alloc_chk(MAX_SPECIES*sizeof(long));
  ...
  for(i=0;i<n_samples;i++) printf("   %s (%li spectra comparisons)\n",sample_name[i],n_compared_spectra[i]);
  ...
  for(x=0;x<n_samples;x++)
    {
      ...
      for(y=0;y<n_samples;y++) fprintf(output_file,",%li",A_gt_cutoff[x][y]);
      fprintf(output_file,"\n"); fflush(output_file);
    }
  fclose(output_file);
  ...
  for(i=0;i<n_species;i++) printf("   species %li -> %s\n",i+1,species_name[i]);
